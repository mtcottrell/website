...
(function ($) {
	...
	mejs.MepDefaults = {
		...
		// features to show
		features: ['seekbackward','playpause','seekforward','current','progress','duration','tracks','volume','fullscreen'],
		// only for dynamic
		isVideo: true,
...
(function($) {
	...
	$.extend(mejs.MepDefaults, {
		seekforwardText: ('Forward 30 seconds')
	});

	// SEEK FORWARD BUTTON
	$.extend(MediaElementPlayer.prototype, {
		buildseekforward: function(player, controls, layers, media) {
			var 
				t = this,
				seekforward = 
				...
				.click(function(e) {					
					...
					if (!isNaN(media.duration) && media.duration > 0) {
							var newTime = Math.max(media.currentTime + player.options.defaultSeekForwardInterval(media), 0);
							media.setCurrentTime(newTime);
					}
					return false;
				});
...
(function($) {
	...
	$.extend(mejs.MepDefaults, {
		seekbackwardText: ('Backward 15 seconds')
	});

	// SEEK BACKWARD BUTTON
	$.extend(MediaElementPlayer.prototype, {
		buildseekbackward: function(player, controls, layers, media) {
			var 
				t = this,
				seekbackward = 
